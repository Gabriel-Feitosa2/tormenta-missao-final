...
const INITIAL_PLAYERS = [
  { id: "player-1", name: "Aragão", characterName: "Leon" },
  { id: "player-2", name: "Ju", characterName: "Fiore" },
  { id: "player-3", name: "Zelele", characterName: "Nero" },
  { id: "player-4", name: "Brunão", characterName: "Salatiel" },
  { id: "player-5", name: "Myu", characterName: "Alori" },
];
...
const RIZZELENA_TABLE = [
  ...
  {
    roll: 3,
    effect: "+2 em teste de resistências até o fim da cena",
    accumulable: false,
  },
  {
    roll: 4,
    effect: "+1d6 em um teste à sua escolha até o fim do dia",
    accumulable: true,
  },
  { roll: 5, effect: "+35 PV e +10 PM temporários", accumulable: true },
  {
    roll: 6,
    effect: "-1 de custo de PM para habilidades de classe",
    accumulable: false,
  },
];
...
const LISANDRA_OPTIONS = [
  {
    id: "heal_50",
    title: "Curar 50 PV",
    description: "Cura 50 pontos de vida do personagem.",
  },
  {
    id: "mana_5",
    title: "Recuperar 5 PM",
    description: "Recupera 5 pontos de mana.",
  },
  {
    id: "tests_2",
    title: "+2 em Testes",
    description: "Ganha +2 em todos os testes pela rodada.",
  },
  {
    id: "attacks_4",
    title: "+4 em Ataques",
    description: "Ganha +4 em testes de ataque pela rodada.",
  },
  {
    id: "damage_burst",
    title: "Explosão Natural (8d8+15)",
    description:
      "Causa 8d8+15 de dano em alcance curto (Aço Rubi ignora RD de Lefeu).",
  },
];
...
class StateManager {
      ...
      try {
        ...
        if (!this.state.combatInstance) {
          this.state.combatInstance = {
            id: 1,
            name: "Combate 1: Entrada na Tormenta",
            active: true,
          };
        }
        ...
        console.error(
          "[StateManager] Erro ao ler session.json, recriando...",
          err,
        );
      }
    }
      ...
      combatLogs: [
        {
          ...
          message:
            "Painel de Parceiros de Tormenta inicializado. Que os deuses tenham piedade de Arton.",
        },
      ],
      lastWillTest: null,
      lastWoundsTest: null,
  ...
  updatePlayer(playerId, updates) {
    ...
    if (updates.name !== undefined)
      player.name = updates.name.trim() || player.name;
    if (updates.characterName !== undefined)
      player.characterName =
        updates.characterName.trim() || player.characterName;
  ...
  claimNpc(playerId, npcId) {
    ...
    if (npc.ownerId && npc.ownerId !== playerId) {
      ...
      throw new Error(
        `Este parceiro já pertence a ${currentOwner ? currentOwner.name : "outro jogador"}.`,
      );
    }

    const currentOwnedCount = Object.values(this.state.npcs).filter(
      (n) => n.ownerId === playerId,
    ).length;
    if (currentOwnedCount >= 8 && npc.ownerId !== playerId) {
      throw new Error(
        "Limite máximo de 8 parceiros atingido para este jogador.",
      );
    }
  ...
  transferNpc(fromPlayerId, toPlayerId, npcId) {
    ...
    if (!fromPlayer || !toPlayer)
      throw new Error("Jogador de origem ou destino inválido.");

    const npc = this.state.npcs[npcId];
    if (!npc) throw new Error("NPC não encontrado.");
    if (npc.ownerId !== fromPlayerId)
      throw new Error("O NPC não pertence a este jogador.");

    const targetOwnedCount = Object.values(this.state.npcs).filter(
      (n) => n.ownerId === toPlayerId,
    ).length;
  ...
  rollWillTest({ dc, globalBonus, targetMode = "active" }) {
    const targetDC =
      dc !== undefined ? parseInt(dc, 10) : this.state.willTestDC;
  ...
  healNpc(npcId, healerInfo = "Cura / Magia") {
    ...
    if (npc.status === "dead")
      throw new Error(
        "Não é possível curar um parceiro morto com cura normal.",
      );
  ...
  rescueWithHealer(curadorId, woundedNpcId, actorName = "Jogador") {
    ...
    if (!curador || !wounded)
      throw new Error("Curador ou parceiro ferido não encontrado.");
    if (!curador.isHealer)
      throw new Error(`${curador.name} não é um parceiro Curador.`);
    if (curador.status !== "alive")
      throw new Error(`${curador.name} precisa estar ativo para resgatar.`);
    if (wounded.status !== "wounded")
      throw new Error(`${wounded.name} não está ferido.`);
  ...
  rollTurnEndInjury(playerId) {
    ...
    if (playerAlivePartners.length > 0 && d8Roll <= injuryThreshold) {
      ...
      const randomIndex = Math.floor(
        Math.random() * playerAlivePartners.length,
      );
    ...
    this.addLog({
      ...
      details: {
        ...
        usedCount,
        injuryThreshold,
        injuredNpc: injuredNpc ? injuredNpc.name : null,
      },
    });
    ...
    return {
      ...
      injuryThreshold,
      injuredNpc,
      totalActivePartners: playerAlivePartners.length,
    };
  }
  ...
  rollCombatWounds() {
    ...
    const woundedNpcs = Object.values(this.state.npcs).filter(
      (n) => n.status === "wounded",
    );
  ...
  rollNpcAttack(npcId, actorName = "Jogador") {
    ...
    if (!npc) throw new Error("NPC não encontrado.");
    if (!npc.attackDice)
      throw new Error(`${npc.name} não possui fórmula de ataque.`);
    if (npc.status === "dead" || npc.status === "withdrawn") {
      throw new Error(
        `Parceiro ${npc.status === "dead" ? "morto" : "fora de combate"} não pode atacar.`,
      );
    }
  ...
  toggleBorusWerewolf(npcId, active) {
    ...
    this.addLog({
      ...
      title: npc.borusWerewolf
        ? "🐺 Borus Transformou-se em Lobisomem!"
        : "Borus Voltou à Forma Humana",
  ...
  selectLisandraOption(actorName = "Jogador", optionId) {
    ...
    this.addLog({
      ...
      message: `${actorName} escolheu para seu turno: ${opt.title} (${opt.description})${
        extraRoll
          ? ` -> Rolou [${extraRoll.rolls.join(", ")}] + 15 = ${extraRoll.total} de Dano de Natureza!`
          : ""
      }`,
